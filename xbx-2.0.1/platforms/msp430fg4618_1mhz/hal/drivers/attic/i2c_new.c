...
#if 0
//TI stuff is weird, TX interrupt vector actually receives.
...
interrupt(USCIAB0RX_VECTOR)  USCIAB0RX_ISR(void)
{
    if(UCB0STAT&UCSTTIFG){		//start condition?
        UCB0STAT &= ~UCSTTIFG;  //yes: clear start flag
        P3OUT ^= BIT4;          //Toggle once to signify start
    }
    if(UCB0STAT&UCSTPIFG){
        UCB0STAT &= ~UCSTPIFG;
        P3OUT ^= ~BIT4;        //Toggle twice ot signify stop
        P3OUT ^= ~BIT4;

        IE2 &= (UCB0RXIE | UCB0TXIE);//Disable interrupts until 
                                     //callback run
        I2cRxStop = 1;         //Enable Rx handler
